...
const UINT8 DestIPAddr[4] = {192, 168, 1, 123};
const UINT8 IPRawProto = 0x01;
...
void InitSocketParam(void)
{
    ...
    memcpy(SockInf[0].IPAddr, DestIPAddr, sizeof(DestIPAddr)); /* Copy the destination IP address */
    SockInf[0].ProtoType = PROTO_TYPE_IP_RAW;                  /* Set to IP RAW mode */
}
...
void CH395SocketInitOpen(void)
{
    ...
    CH395CMDSetSocketIPRAWProto(0, IPRawProto);         /* Set the protocol field */
...
void CH395SocketInterrupt(UINT8 sockindex)
{
    ...
    if (sock_int_status[sockindex] & SINT_STAT_RECV) /* Receive interruption */
    {
        ...
        if (len > 512)
            len = 512;
        CH395CMDGetRecvData(sockindex, len, MyBuffer); /* Read data */
