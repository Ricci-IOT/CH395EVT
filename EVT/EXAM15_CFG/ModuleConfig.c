...
void LongToChar(u8 *buf, UINT32 dat)
{
    UINT8 *p;
    p = buf;
...
/**********************************************************************************
 * Function Name  : ParseConfigbuf
 * Description    : Configuration command parsing
 * Input          : buf
 * Output         : None
 * Return         : None
 **********************************************************************************/
UINT8 ParseConfigbuf(u8 *buf)
{
    UINT16 i;
    UINT32 len;
    pnet_comm net = (pnet_comm)buf; // net communication structure
    pmodule_cfg mdcfg = (pmodule_cfg)net->dat;
    pmodule_cfg mCurrentCfg = (pmodule_cfg)Configbuf;

    i = memcmp(net->flag, NET_MODULE_NAME, 15);
    if (i)
        return 0;
    ...
    case NET_MODULE_CMD_RESET:
        ...
        strcat((char *)mdcfg->module_name, (char *)MODULE_NAME); // Default configuration
